...
class ReActAgent:
    ...
    def __init__(self):
        ...
        self.schema_tool = StructuredTool.from_function(
            ...
            coroutine=self.get_table_schema,
    ...
    async def call_get_schema(self, state: MessagesState):
        ''' Get table columns names '''
        #print("====> get schemas:", state)
    ...
    async def get_table(self, state: MessagesState):
        ''' Get table name'''
        ...
        response = AIMessage(f"Available tables: {tool_message}")
        return {"messages": [tool_call_message, tool_message, response]}
        
    async def generate_query(self, state: MessagesState):
        #print("====> generate_query:", state["messages"])
        system_message = {
            "role": "system",
    ...
    def create_custom_toolmessage(self, result, call_id: str) -> ToolMessage:
        ''' Build custom ToolMessage from MCP Tool Result '''
        #print("====> ToolResult:", result)
        if result.content and hasattr(result.content[0], 'text'):
            content_text = result.content[0].text
        else:
            content_text = "No response from tool." 
       
        toolmsg = ToolMessage(content=content_text, tool_call_id=call_id)
        return toolmsg

    # MCP Tool call 
    async def get_table_name(self, name: str) -> str:
        """ Returns the table name """
        #print(f" ====> Get table name tool {name}")
        async with self.client:
            result = await self.client.call_tool("get_tables_name")
        toolmsg = self.create_custom_toolmessage(result, 'call_id_abc123')
        return toolmsg
    
    # MCP Tool call
    async def get_table_schema(self) -> list:
        """ Get the table schema """
        #print(" ====> Get table schema tool")
        async with self.client:
            result = await self.client.call_tool("get_table_schema", {"t_name": "books"})
        return result
    ...
    async def execute_sql_query(self, query: str) -> str:
        ...
        async with self.client:
            result = await self.client.call_tool("execute_sql_query_async", {"query": query})
        return result
